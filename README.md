...
現在、memcalc の内部処理方法について、１から書き直すに相当する大幅な改修を行っています。
そのため、
commit fb17c83cc8d00e6b096255a6b16449b2cb3aad59 あたり以降から現在までのコミットにおいて、
いくつかの機能が動作しなくなっています。
また、関数呼び出しの文法も大幅に変更されています。

文法の変更点:

    ・関数の引数の書式が変わりました。
    新しい書式では括弧が不要となり、カンマの代わりにスペースを用います。
        例:
            旧 f(a,b,c);
            新 f a b c;

    ・配列宣言で確保されるサイズが +1 増えました。
        例:
            旧 a[10] = 1; b = 2; print(a[10]);
            この場合にどちらも 2 が出力されていました。

            新 a[10] = 1; b = 2; print b;
            この場合に 1 が出力されます。
            そして print(a[11]) が print(b) に相当します。

    ・ラムダ抽象を変数へ代入でき、その変数をラムダ関数として使用できます。
    （ただし、現状では変数スコープにバグがあるはずです）
    （また、print等の関数で非数字の場合の扱いが未定義だったりと、バギーな状態です）
        例:
            a = (\x . x + 1);
            これで a は (\x . x + 1) というラムダ抽象を司ります。

            a 2;
            これは (\x . x + 1) 2 と同じ意味です。すなわち

            print (a 2);
            は 2 + 1 の結果（すなわち 3）を印字します。

            また、ラムダ抽象への引数に、ラムダ抽象を渡すことも可能です。（高階関数）
            例:
                a = (\f . f);
                b = (\x . x * x);
                c = a b;
                この場合の c は (\f . f) (\x . x * x) と同様です。
                この式はすなわち (\x . x * x) というラムダ抽象を返すという意味です。
                よって c 3 とすれば、それは (\x . x * x) 3 と同様の意味です。

                print (c 3)
                従って、これは 9 を印字します。

                また、もう少し複雑な例として:（←これ動かない）
                    a = (\f . f z)
                    b = (\x . x * x)
                    c = a b
                    これは (\x . x * x) z と同じ状態ですが、
                    z がまだ未定義だった場合は式を返し、
                    z が”数値”によって定義済みの場合は z * z の計算結果を返します。
                    （この場合はzがラムダ抽象の場合はエラーにすべきですが、その辺はまだ未整備です）

    ・中括弧で囲んだ範囲の変数スコープを局所化できます。
        例:
            x = 1;
            {
                x = 2;
            ...
            print x;
            この場合、最初の print で 3 が印字され、次の print で 2、最後の print で 1 が印字されます。
            すなわち x はスコープ内で再定義された場合は、局所変数として上書きされます。

            また、スコープ内で上書きしなかった変数ならば、同様にアクセスできます。
            例:
                y = 1;
                {
                    {
                ...
                print y;
                この場合、どの print も y へアクセスでき、1 を印字します。

正常動作しなくなる点:

    ・ジャンプ系（goto, gosub, return）が全滅です。動作しません。（このまま廃止するかもしれません）

（このような状態のため、以下の README.md に書いてある内容のいくつかは、現状と乖離しています）
（また、sample/ 以下のファイルも、現状では、いくつかを動作させられません）

***


限定的にですが、ラムダ関数を実装しました。
ただし、引数を与えない状態のラムダ関数自体を変数へと代入することはできません。
現状では必ずラムダ関数は式の中で一息（引数も与えて、”値を得られる完全な状態”）として書く必要があります。
また当然ですが、ラムダ関数の中で再定義された変数は局所変数となり、それはグローバル変数をオーバーライドします。



ユーザーによるサブルーチン定義は未実装ですが、代用として、申し訳程度に gosub によるラベルジャンプと、return による復帰を行えます。
ただし、gosub と return によるサブルーチンでは、変数のスコープは大域変数のままなので、つまり goto でジャンプしてるだけなのと同じです。
サブルーチンによる再帰的な処理を行うには不向きです。
将来的にはサブルーチン内で、スタック変数によって大域変数をオーバーライドできるようにしたいと考えてます。

しかしながら、結局これら goto, gosub, return などは一時的な実装にすぎません。
最終的な実装の目標としては goto, gosub, return は廃止し、ラムダ関数を実装し、これを第一級オブジェクトとして扱えるようにしたいと野望を持ってます。
限定的なラムダ関数の実装はできましたが、引数の与えられてない状態の、関数のみの状態での変数への代入に関しては、それは自分の知識レベルではハードルが高すぎて、今のところ具体的な実装方法が漠然としか見えない状態です。
（漠然としか見えていないので、具体的に詳細に詰めて思考するとすぐにボロが出て、破綻ポイントがいくつも見つかって、結局１から再思考しなおす。をずっと繰り返してる状態です。現状、実装の目処はまったく立ってません）
...
### 言語の全機能・仕様の説明：
        ...
        （ラムダ関数内で再定義された変数以外の）全ての変数は大域変数となります。
        型は double による複素数型のみです。
        ...
        等号および不等号の誤差許容値について:
                == は実際には等号ではなく ≈　です。
                a,bの差が 1e-14　よりも小さい場合、すなわち a,b がほぼ等しい場合に真を返します。
                /= は == の否定なので同様に、a,b がほぼ異なる場合に真を返すこととなります。

                もしも == において完全に一致のみを真としてしまうと、
                memcalcでは数を全て内部的に倍精度浮動小数（誤差が避けられない）として扱うので、
                完全に一致のみを真と判定していてはシビアすぎてしまいます。

                例えば (1/3)*3 と (1/5)*5 は数学的にはどちらも 1 ですが、
                浮動小数点数であれば、これらは誤差によって等しい 1 だとは判定されない場合もあり得ます。
                そこで、適当な桁以下の誤差は無視して判断するように変更せざるをえませんでした。

                具体的には現状では、a,b の差の絶対値が 1e-14 以下であれば、
                それらを等しい値として見なします。

                もっと良い方法としては、
                a,bの有効桁数における関係性に応じて、誤差の許容値を可変的にすべきだと思います。
                現状では、あまり深くは考えずに、単に差の許容値を 1e-14 で固定してあります。
                そのため、非常に大きな数同士の比較では許容値が不十分であり、また逆に、
                非常に小さな数同士の比較においては許容値が過剰であることになってしまうはず。
                これは問題です。

                これはもっと真面目にa,bの有効桁数の関係性に応じて、許容値を可変的に導出すべきかと思います
                が、しかし、現状ではその具体的な実装は後回しとなってます。
                理由は、ただ単にそれに関しての有効な計算式を知らないからです。
        ...
        ラベルは goto または gosub のジャンプ先の行として使います。
        ...
        gosub ラベル名;
        によって、その行へジャンプできます。（最後の ; は必要です）
        goto との違いは、gosub でジャンプした場合は、ジャンプする際にプログラム位置をスタックへ記録します。
        そして return; によって元の位置へ復帰できます。
        これはサブルーチンの真似事を行う場合に、gotoよりかは多少便利というだけで、基本的に goto と同じことです。

        すなわち、変数のスコープは大域変数のままです。（局所変数（スタック変数）の概念は依然として未実装です）
        例：
                a = 0;
                gosub F;
                gosub F;
        ...
        (\変数名 . 式) 引数;
        ラムダ関数を使用できます。（ただし、引数を与えられて、値を完結して得られる状態でのみ）
        変数名は、ラムダ関数内のスコープ以下に限定された変数となり、そこへ引数の値が代入されて運用されます。
        その引数を用いて、式が実行されて、結果の値を返します。
        ...
        例:
                ...
                a = (\x . x + y) 123;
                print(x);
                この場合はラムダ関数内での x は 123 と成り、a は 125 となりますが、
                しかし print(x); は 1 となります。
                なぜなら、ラムダ関数内の x と、外の x とでは、これらは別の変数だからです。
                もちろん、ラムダ関数内の y に関しては、再定義されてないので外も内も同じです。
                ...
                例:
                        ...
                        この場合は概念としては
                        a = (\x . x + 3) 4
                        と同様で、すなわち変数 y に 3 が拘束された状態のラムダ関数へ、
                        引き数として 4 を渡してる状態に相当します。

                        さらに深い入れ子も可能です。

        現状では、ラムダ関数へ引数を与えてない状態では、変数へ代入することはできません。
        （すなわち値が確定しない状態では、変数へは代入できません）
        例:
                a = (\x . x + 1);
                これに関しては現状では未実装です。代入できません。

                将来的にはこれを有効として a を関数として扱えるようにし、同様に
                b = a;
                や
                b = a 2;
                などを行えるようにしたいと思ってますが、実相の目処は立ってません。
        ...
        (条件式) -> 式 :: 式
        数式中に含めることのできる条件分岐を追加しました。
        if 文とは異なり、これは結果として値を返すので、数式中に含めることができます。
        また、if文とは異なり、分岐結果は命令実効ではなく式実効なので、現状では組み込み関数は式に含められません。
        (現状では関数は式ではなく命令として扱われます)
        例:
                x = 1;
                (1 == x) -> x + 1 :: x - 1;
                条件式は括弧で囲む必要があります。

                また、現状では内部的には分岐の式を両方とも評価しているので、if文よりも処理に無駄が多いです。
                （if文では分岐先の命令しか実効しないので効率的です）
                （現状ではif文に比べると、実装面において問題が多い状態です）
