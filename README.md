...
a の共益複素数を得たい場合(conjugate)は ~a とします。これは変数の上にバーを書く記法を模しています。結果は実数です。

a の極座標における偏角を得たい場合は、arg a; とします。結果は実数です。
...
現時点では指数関数と三角関数が未実装なのでせっかくの複素数のおいしい所を生かしきれません。
...
### 言語の全機能・仕様の説明：
        ...
        a の複素共役(conjugate)を得たい場合は ~a とします。結果は実数です。
        a の極座標における偏角(argument)を得たい場合は arg a とします。結果は実数です。
        a の複素平面における実数値を得たい場合は Re a とします。（名前の大文字・小文字は区別します）
        a の複素平面における虚数値を得たい場合は Im a とします。（名前の大文字・小文字は区別します）
        例：
                a = 1 :+ 1;
                b = (|a|) :< (arg a);
        ...
        ただし、比較は実数のみで比較します。（虚数は無視して判断します）。
